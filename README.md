# طلب 9: محاكاة 100 مستخدم (JMeter )
...
1. **- مستخدم اختباري** برصيد كافي (مثلاً 1,000,000) وعدّلوا `EMAIL`/`PASSWORD` بالـ Test Plan (User Defined Variables) ليطابقوه.
...
بعدين:
1. بملف الـ Test Plan، فعلوا عنصر **"InfluxDB Backend Listener (for Grafana)"** (غيّروا `enabled="false"` لـ `enabled="true"`، أو من GUI كبسة يمين → Toggle).
